...
with tab1:
    ...
    r2_gph = st.sidebar.slider("Select r", 2.5, 4.0, 3.5)
    a_gph = st.sidebar.slider("Select a", 0.0, 2.0, 1.0)
    a_gph_iterations = st.sidebar.number_input("Iterations", value=50, min_value=1, max_value=500)
    
    st.sidebar.markdown("---")
    st.sidebar.header("Asymmetric Map: Bifurcation Parameters")
    a_r_min = st.sidebar.slider("Min r", 2.5, 4.0, 2.5)
    a_r_max = st.sidebar.slider("Max r", 2.5, 4.0, 4.0)
    a_n_iterations = st.sidebar.number_input("Iterations per r", value=1000)
    a_n_discard = st.sidebar.number_input("Discard initial", value=100)

    st.sidebar.markdown("---")
    st.sidebar.header("Asymmetric Map: Time Series Analysis Paramters")
    a_r_val = st.sidebar.slider("Select r for Time Series", 2.5, 4.0, 3.5)
    a_n_time_steps = st.sidebar.number_input("Time steps", value=100)

    st.header(f"Dynamical System Plot (r = {r2_gph})")
    ...
    def generate_cobweb_plot(r, steps):
        ...
        # Plot the system function and equilibrium line
        ax.plot(x_vals, r * x_vals * (a_gph - x_vals), 'r', label="f(x) = r*x(1-x)")
        ax.plot(x_vals, x_vals, 'k--', label="y = x")
    ...
    if st.button("Generate Asymmetric Graph"):
        fig_cobweb = generate_cobweb_plot(r2_gph, a_gph_iterations)
        st.pyplot(fig_cobweb)
    ...
    if st.button("Show Asymmetric Time Series"):
        data = generate_time_series(a_r_val, a_n_time_steps)
    ...
    if st.button("Generate Asymmetric Diagram"):
        with st.spinner("Calculating..."):
            r_vals, x_vals = generate_bifurcation_data(a_r_min, a_r_max, a_n_iterations, a_n_discard)
